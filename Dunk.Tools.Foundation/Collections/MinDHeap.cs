...
{
    /// <summary>
    /// Represents a D-Ary minimum heap of objects.
    /// </summary>
    /// <typeparam name="T">The type of elements in the heap.</typeparam>
    public sealed class MinDHeap<T> : DHeap<T>
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool MinCompare(int result)
        {
            return result > 0;
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="MinDHeap{T}"/> class that is empty
        ...
        /// </exception>
        public MinDHeap(int order, int capacity, IComparer<T> comparer = null)
            : base(order, MinCompare, capacity, comparer)
        {
        }
        ...
        public MinDHeap(int order, IComparer<T> comparer = null)
            : this(order, 0, comparer)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="MinDHeap{T}"/> class that contains 
        ...
        /// <exception cref="System.ArgumentOutOfRangeException"><paramref name="order"/>is less than or equal to one.</exception>
        public MinDHeap(int order, IEnumerable<T> collection, IComparer<T> comparer = null)
            : base(order, MinCompare, collection, comparer)
        {
        }
    }
}
